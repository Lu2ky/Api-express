...
//	------------------------ FUNCIONALIDADES DEL LDAP ------------------------ //
  ...
  async addPersonalComment(N_idHorario, T_comentario) {
    const url =
      "http://" +
      process.env.API_ADDR +
      ":" +
  ...
  // Actualizar estado de recordatorio
  async updateStateReminder(idToDo, newState) {
    const url =
      "http://" +
      process.env.API_ADDR +
      ":" +
  ...
  // Agregar notificación
  async addNotification(idToDo, name, desc, issueDate) {
    const url =
      "http://" +
      process.env.API_ADDR +
      ":" +
  ...
  // Agregar correo
  async addEmail(idToDo, issue, content, issueDate) {
    const url =
      "http://" +
      process.env.API_ADDR +
      ":" +
  ...
  async authuser(user, pass) {
    const url =
      "http://" + process.env.API_ADDR + ":" + process.env.API_PORT + "/auth";
